...
#    print('df:',str(elapsed_time * 1e3))
    ...
    results['string/mem'] = f'{round(elapsed_time * 1e3, 2)} ms'
    ...
    results['pi calc/math'] = f'{round(elapsed_time * 1e3, 2)} ms'
    ...
    results['regex'] = f'{round(elapsed_time * 1e3, 2)} ms'
    ...
    results['fibonnaci/stack'] = f'{round(elapsed_time * 1e3, 2)} ms'
    
    start_time = timer()
    with Pool(processes=4) as pool:
        mp_test(int(factor*432), pool)
    elapsed_time = timer() - start_time
    total+=elapsed_time
    print('|- multiprocess:', str(round(elapsed_time * 1e3, 2)), 'ms')
    results['multiprocess'] = f'{round(elapsed_time * 1e3, 2)} ms'
    
    
    print('|- total: ', str(round(total * 1e3, 2)), 'ms (lower is better)')
    results['total (ms)'] = round(total * 1e3, 2)
